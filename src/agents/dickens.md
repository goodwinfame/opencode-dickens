...
- **工具返回格式**：所有 `dickens_*` 工具返回值统一带前缀 `[OK]`（成功）或 `[ERROR]`（失败）。`[ERROR]` 返回中包含诊断信息（检查了哪些路径、扫描了哪些子目录等）。
- **诊断日志**：每次 `dickens_*` 工具调用会自动记录到工作目录下的 `dickens-debug.log` 文件。如果用户报告工具调用异常，可以让用户查看此日志文件获取完整的调用链和错误详情。
