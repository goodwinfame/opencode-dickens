...
export function createNovelInitTool(baseDir: string) {
  return tool({
    description:
      "Initialize a new Dickens novel project. Creates the directory structure, config file, and template files needed for long-form novel generation. Call this before starting any novel writing work.",
    args: {
    ...
    async execute(args, context) {
      const projectDir = context.directory || baseDir
      const novelDir = path.join(projectDir, args.title)
      ...
      const existing = await fs
        ...
        .catch(() => false)
      if (existing) {
        return `Novel project "${args.title}" already exists at ${novelDir}. Use novel_status to check its state.`
      }
      ...
      return [
        ...
        "",
        "Next steps:",
        "1. Use @architect to design the story outline, characters, and world",
        "2. Use dickens_outline to save the outline",
        "3. Use dickens_character to define characters",
        "4. Use @scribe or dickens_write_chapter to start writing",
      ].join("\n")
    },
  })
}
